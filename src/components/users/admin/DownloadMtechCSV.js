...
export const DownloadCSV = ({ type, list, handleExcellentListSave }) => {
  const [mtechMentees, setMtechMentees] = useState([]);
  ...
  const handleDownloadMentorMenteeCSV = () => {
    try {
      const sortedMtechMentees = [...mtechMentees].sort((a, b) => {
      ...
      const mappedMtechMentees = sortedMtechMentees.map((mentee) => {
        ...
        console.log(mentee.mentorDepartment, mentorDepartmentOption);
      ...
      const csvDataMtech = convertToCSV(mappedMtechMentees);
      downloadCSV(csvDataMtech, "Mentor-Mentees_Mtech_AY_.csv");
    } catch (error) {
