...
export default function SendMail({
  ...
  const sendConsentMail = async () => {
    try {
      const response1 = await axios.post(
        "http://smpportal.iiitd.edu.in/api/sendConsentForm/",
        JSON.stringify({
          subject: mailSubject,
          body: mailBody,
  ...
  const handleMentorMenteeMapping = async () => {
    try {
      const response = await axios.post(
        "http://smpportal.iiitd.edu.in/api/createMentorMenteePair/",
        JSON.stringify({
          subject: mailSubject,
          body: mailBody,
          Id: newlySelectedStudents,
  ...
  const fetchMailDetails = useCallback(async (formType) => {
    try {
      ...
      const response = await axios.post(
        "http://smpportal.iiitd.edu.in/api/getMailSubjectAndBody/",
        JSON.stringify({
          type: key,
        })
      );
