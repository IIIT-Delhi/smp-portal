...
from backend.server.view.mentee_views import delete_all_mentees, get_all_mentees
from backend.server.view.mentor_views import get_all_mentors, get_mentor_by_id
...
class AddCandidateTestCase(TestCase):
    ...
    def test_add_candidate_missing_required_field(self):
        ...
        self.assertEqual(response.json(), {'error': 'Missing required field: name'})
...
class TestAnonymous(TestCase):
    ...
    def test_returns_id_for_mentee(self):
        # Calling the function under test
        response = self.client.post('http://127.0.0.1/getIdByEmail/', json.dumps({'email': 'sarthak20403@iiitd.ac.in', 'role': 'mentee'}).encode('utf-8'), content_type='application/json')
        self.assertEqual(response.status_code, 200)
        response=json.loads(response.content.decode('utf-8'))
        # Asserting the response
        self.assertEqual(response["id"],-1)
    ...
    def test_returns_mentees_with_mentor_details_if_request_method_is_get(self):
        ...
        # Act
        response = delete_all_mentees(request)
        # Assert 
        self.assertEqual(response.status_code, 200)    
    ...
    def test_valid_post_request_with_id_parameter(self):
        ...
        with self.assertRaises(IndexError):
            self.client.post('http://127.0.0.1/getMentorById/', json.dumps({"id": 1}).encode('utf-8'), content_type='application/json')
    
       # Returns a JSON response with "Invalid request method" message when a POST request with missing 'id' parameter is received
    def test_valid_post_request_with_id_parameter(self):
        # Act
        with self.assertRaises(IndexError):
            self.client.post('http://127.0.0.1/getMentorById/', json.dumps({}).encode('utf-8'), content_type='application/json')
